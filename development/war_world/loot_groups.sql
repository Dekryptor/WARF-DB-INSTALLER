...
CREATE TABLE `loot_groups` (
  ...
  `Name` text,
  `DropEvent` tinyint(3) unsigned DEFAULT NULL,
  `CreatureID` int(10) unsigned DEFAULT NULL,
  `CreatureSubType` smallint(5) unsigned DEFAULT NULL,
  `DropChance` float DEFAULT NULL,
  `DropCount` tinyint(3) unsigned DEFAULT NULL,
  `ReqGroupUsable` tinyint(3) unsigned DEFAULT NULL,
  `ReqActiveQuest` smallint(5) unsigned DEFAULT NULL,
  `SpecificZone` smallint(5) unsigned DEFAULT NULL,
  PRIMARY KEY (`Entry`)
